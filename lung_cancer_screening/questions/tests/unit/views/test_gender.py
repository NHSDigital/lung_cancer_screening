...
from lung_cancer_screening.questions.models.gender_response import GenderValues
...
class TestPostGender(TestCase):
    ...
    def test_creates_a_gender_response(self):
        response_set = ResponseSetFactory.create(user=self.user, eligible=True)

        self.client.post(reverse("questions:gender"), self.valid_params)

        response_set.refresh_from_db()
        self.assertEqual(
            response_set.gender_response.value, self.valid_params["value"]
        )
