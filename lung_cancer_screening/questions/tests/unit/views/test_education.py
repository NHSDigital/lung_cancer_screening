...
from lung_cancer_screening.questions.models.education_response import EducationValues
...
class TestPostEducation(TestCase):
    ...
    def test_creates_an_education_response(self):
        ...
        self.client.post(reverse("questions:education"), self.valid_params)

        response_set.refresh_from_db()
        self.assertEqual(
            response_set.education_response.value, self.valid_params["value"]
        )

    def test_redirects_to_respiratory_conditions(self):
        ResponseSetFactory.create(user=self.user, eligible=True)
