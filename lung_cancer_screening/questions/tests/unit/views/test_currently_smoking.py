...
class TestGetCurrentlySmokingCigarettes(TestCase):
    def setUp(self):
    ...
    def test_redirects_if_the_user_is_not_logged_in(self):
        ...
        response = self.client.get(
            reverse("questions:currently_smoking_cigarettes")
        )

        self.assertRedirects(response, "/oidc/authenticate/?next=/currently-smoking-cigarettes", fetch_redirect_response=False)
    ...
    def test_responds_successfully(self):
        ...
        response = self.client.get(reverse("questions:currently_smoking_cigarettes"))

        self.assertEqual(response.status_code, 200)


@tag("CurrentlySmokingCigarettes")
class TestPostCurrentlySmokingCigarettes(TestCase):
    def setUp(self):
        self.user = login_user(self.client)
    ...
    def test_creates_a_currently_smoking_cigarettes_response(self):
        response_set = ResponseSetFactory.create(user=self.user, eligible=True)

        self.client.post(reverse("questions:currently_smoking_cigarettes"), self.valid_params)

        response_set.refresh_from_db()
        self.assertEqual(
            response_set.currently_smoking_cigarettes_response.value, self.valid_params["value"]
        )
